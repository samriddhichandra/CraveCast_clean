...
function Navigation({
    ...
    try {
      ...
      if (status === 429 || code === 'insufficient_quota') {
        setAiErrorMessage(
          ...
          'OpenAI quota exceeded. Add billing/credits to continue generating recipes.'
        );
      } else {
        setAiErrorMessage('Failed to generate recipes. Please try again.');
      }
      setIsLoading(false);
    }
  };
  ...
  // If AI generation fails hard, show a friendly message (but quota/rate-limit uses demo mode server-side).
  if (aiErrorMessage) {
    return (
      <LimitReached
