...
type PaginationAction =
  ...
  | { type: "FETCH_FAILURE"; payload?: { message?: string } }
  | { type: "INCREMENT_PAGE" }
...
function reducer(state: PaginationState, action: PaginationAction): PaginationState {
  switch (action.type) {
    ...
    case "FETCH_FAILURE":
      return { ...state, loading: false, error: action.payload?.message || "Failed to load recipes." };
    case "INCREMENT_PAGE":
      ...
      try {
        ...
        const message =
          error instanceof Error
            ? error.message
            : "Network error while fetching recipes.";
